...
if($address_type->show_country_select) {
	...
	print '<h1>'. \Sprog\Wildcard::get('d2u_address_local_servicepartner') .'</h1>';
	print '<p>'. \Sprog\Wildcard::get('d2u_address_specialists') .'</p>';
	print '<br />';
	...
	if(rex_request('country_id', 'int') > -1 && $country !== false) {
		...
		if(count($country_zip_codes) > 0) {
			...
			if($show_zip_code_field) {
				print '<div class="col-12 col-md-6">';
				$placeholder = $zip_code === false ? \Sprog\Wildcard::get('d2u_helper_module_form_zip') : rex_request('zip_code', 'int');
...
if(count($addresses) > 0) {
	if($address_type->show_country_select) {
		// Only if coutry selection should be available
		print '<div class="col-12"><h2>'. \Sprog\Wildcard::get('d2u_address_nearby') .'</h2><br></div>';
	}
