...
<ul>
	...
	<li>Backend: CSRF-Schutz fuer Modul-Installation, -Update und -Deinstallation auf der Setup-Seite ergaenzt.</li>        <li>Security: Die <code>media-is-in-use</code>-Extension-Points in <code>boot.php</code> verwenden jetzt gebundene Parameter statt SQL-String-Konkatenation mit <code>addslashes()</code>.</li>
        ...
        <li>Security: Modul-Ausgaben (<code>modules/20/1/output.php</code>, <code>modules/20/3/output.php</code>, <code>modules/20/4/output.php</code>, <code>modules/20/6/output.php</code>) härten Leaflet-Popup-Inhalte: Werte werden via <code>json_encode()</code> in JS-Strings übergeben statt mit <code>addslashes()</code>; Lat/Long strikt als <code>(float)</code> gecastet. Weitere Adress-/Namensfelder werden in HTML-Ausgaben mit <code>rex_escape()</code> gehärtet.</li>
        <li>Security: Module 20-3 und 20-6 (Google Maps): API-Key wird vor der Einbettung URL-kodiert; Adress-Marker-Daten (Straße, PLZ, Ort, Kontaktinfo, Land) werden über <code>json_encode()</code> in das JS-Adress-Array geschrieben statt durch String-Konkatenation; Lat/Long und Map-IDs strikt als <code>(float)</code> bzw. <code>(int)</code> gecastet. Verhindert XSS über Adressdaten in den Infofenstern.</li>
        <li>Security: Module 20-1 und 20-4 (Google Maps, BS4): API-Key vor der Einbettung URL-kodiert; das JS-Adress-Array (Geocoder-Adresse, Infofenster-Inhalt) wird jetzt über <code>json_encode()</code> erzeugt und die Einzelwerte (Firma, Kontaktname, Land, PLZ, Ort, Telefon, Mobil) zusätzlich mit <code>rex_escape()</code> gehärtet; Lat/Long strikt als <code>(float)</code> gecastet. Verhindert XSS über Adressdaten im JS-Kontext.</li>
        <li>Security: Auch die HTML-Adresskarte in <code>modules/20/1/output.php</code> wird gegen XSS gehärtet: Firma, Kontaktname, Firmenzusatz, Straße, PLZ/Ort, Mobil, Telefon, Fax sowie E-Mail (mailto-Link, Linktext und Analytics-<code>onClick</code>) werden mit <code>rex_escape()</code> ausgegeben. Die E-Mail-Links in <code>modules/20/2/output.php</code> und <code>modules/20/5/output.php</code> sowie die <code>&lt;option&gt;</code>-Labels (Adresstyp-/Firmen-/Kontaktnamen) in den Input-Formularen von <code>20/1</code>, <code>20/2</code>, <code>20/3</code>, <code>20/5</code> und <code>20/6</code> werden ebenfalls mit <code>rex_escape()</code> gehärtet.</li></ul>
<p>1.6.0:</p>
<ul>
