...
if (!$invalidCsrf && (1 === (int) filter_input(INPUT_POST, 'btn_save') || 1 === (int) filter_input(INPUT_POST, 'btn_apply'))) {
    ...
    $address = new FriendsOfRedaxo\D2UAddress\Address($form['address_id'], (int) rex_config::get('d2u_helper', 'default_lang'));
    ...
    $address->country = new FriendsOfRedaxo\D2UAddress\Country($form['country_id'], (int) rex_config::get('d2u_helper', 'default_lang'));
...
if ((!$invalidCsrf && 1 === (int) filter_input(INPUT_POST, 'btn_delete', FILTER_VALIDATE_INT)) || 'delete' === $func) {
    ...
    $address = new FriendsOfRedaxo\D2UAddress\Address($address_id, (int) rex_config::get('d2u_helper', 'default_lang'));
...
if ('' === $func) {
    ...
    if (\rex::getUser() instanceof rex_user && (\rex::getUser()->isAdmin() || \rex::getUser()->hasPerm('d2u_address[edit_data]'))) {
        $list->addColumn(rex_i18n::msg('status_online'), '<a class="rex-###online_status###" href="' . BackendHelper::getCurrentBackendPage(['func' => 'changestatus', 'entry_id' => '###address_id###'], [], true) . '"><i class="rex-icon rex-icon-###online_status###"></i> ###online_status###</a>');
        ...
        $list->setColumnParams(rex_i18n::msg('delete_module'), ['func' => 'delete', 'entry_id' => '###address_id###'] + $csrfToken->getUrlParams());
